        ...
        ItemFilter: WinJS.Class.define(function ItemFilter(selectors, category) {
            ...
            update: function (bindingList) {
                ...
                if (bindingList._bindingList._lastNotifyLength > 0) {
                    if (container) container.innerHTML = '<div class="filterarea"><div class="filterbar"></div><select class="filterselect"></select></div>';
                } else {
                    if (!navigator.onLine) {
            ...
            _filterChanged: function (element, filterIndex) {
                ...
                if (filterBar) WinJS.Utilities.removeClass(filterBar.querySelector(".highlight"), "highlight");
...
(function () {
    ...
    WinJS.Namespace.define('Class', {
        ...
        /* コンストラクタ。 */
        VideoData: WinJS.Class.define(function VideoData(videoSource, data, label) {
            ...
            if (!data) {
                ...
                switch (videoSource) {
                    ...
                    case VideoSource.YOUTUBE:
                        ...
                        var result = new Class.YoutubeParser(data, label);

                        break;

                    case VideoSource.LOCAL_VIDEO:

                        var result = new Class.FileInformationParser(data, label);
    ...
    WinJS.Namespace.define('Class', {
        ...
        FileInformationParser: WinJS.Class.define(function FileInformationParser(data, label) {
            ...
            for (var i = 0; i < n; i++) {
                ...
                if (!fileInformation[i].isOfType(Windows.Storage.StorageItemTypes.folder)) {

                    var info = fileInformation[i];
                    var url;

                    try {
                        url = URL.createObjectURL(info.thumbnail, { oneTimeOnly: true });
                    } catch (error) {
                        url = Settings.ALT_IMAGE;
                    }

                    var pathSplit = info.path.split('\\');
...
(function () {
    ...
    WinJS.Namespace.define('Class', {
        ...
        /* コンストラクタ。 */
        YoutubeParser: WinJS.Class.define(function YoutubeParser(json) {

            var source = VideoSource.YOUTUBE;
            var n = json.feed.entry ? json.feed.entry.length : 0;
            ...
            var result = n > 0 ? new Class.SearchResultGroup(source) : [];
            ...
            for (var i = 0; i < n; i++) {
                ...
                if (object.media$group.media$content) {
        ...
        FormatForSearchAPI: WinJS.Class.define(function FormatForSearchAPI(source, query) {
            ...
            switch (source) {
                ...
                case 'youtube':
                    ...
                    return {
                        url: "http://gdata.youtube.com/feeds/api/videos?q=" + encodeURI(query) + "&v=2&restriction=JP&safeSearch=strict&lr=ja&alt=json",
                        query: query,
                        source: source
                    }
