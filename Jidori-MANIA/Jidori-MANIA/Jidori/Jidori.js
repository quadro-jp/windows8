...
(function () {
    ...
    function initialize() {
        ...
        if (!Windows.Storage.ApplicationData.current.roamingSettings.values["lastTutorial"]) {
            Windows.Storage.ApplicationData.current.roamingSettings.values["lastTutorial"] = '';
        }
        ...
        function onVideoDataChange(video) {
            ...
            Jidori.log('動画が変更されます ' + Jidori.player);
            ...
            if (primaryMonitor) {
                Jidori.log('プライマリモニタを頭出しします ' + WinJS.Navigation.location);
            ...
            if (secondaryMonitor) {
                secondaryMonitor.addEventListener('ended', onSecondaryMonitorComplete, false);
                if (video != '#') {
                    Jidori.log('セカンダリモニタに動画をセットします ' + WinJS.Navigation.location);
                    secondaryMonitor.src = video;
                    secondaryMonitor.volume = 0;
        ...
        if (videoData) {
            ...
            if (related.length > 0) {
                ...
                if (related[0].folder) {
                    ...
                    MyDocument.tutorialLibrary.getFilesAsync().then(function (item) {

                        var n = item.length;
                        var relatedVideoData;

                        for (var i = 0; i < n; i++) {
                            if (item[i].name == related[0].src) {
                                relatedVideoData = item[i];
