    ...
    function initialize() {
        ...
        function checkRelatedData() {
            ...
            tableList.forEach(function (name) {
                ...
                if (name.indexOf('.mp4') != -1) checkLocalVideoRelatedData(name);

                /* 関連動画情報のキーに.mp4を含む場合は、ローカル動画として処理 */
                if (name.indexOf('.mp4') == -1) checkWebserviceVideoRelatedData(name);
            });
        }
        ...
        function checkLocalVideoRelatedData(name) {
            ...
            /*
            folder.getFileAsync(name).then(function (storageFile) {
                if (name == storageFile.name) Jidori.log(name + 'の関連情報が見つかりました。');
            }, function () {
                Jidori.log(name + 'の関連情報が見つかりませんでした。テーブルを消去します。');
                Storage.deleteRelationData(name);
            });
            */
        }
        ...
        function checkWebserviceVideoRelatedData(folder, name) {
            ...
            /*
            folder.getFileAsync(name).then(function (storageFile) {
                if (name == storageFile.name) console.log(name + 'の関連情報が見つかりました');
            }, function () {
                Jidori.log(name + 'の関連情報が見つかりませんでした。テーブルを消去します');
                Storage.deleteRelationData(name);
            });
        ...
        function onVideoDataChange(video) {
            ...
            if (primaryMonitor) {
                //primaryMonitor.addEventListener('seeked', onSeekingPrimaryMonitor);
            ...
            if (secondaryMonitor) {
                //secondaryMonitor.addEventListener('seeked', onSeekingSecondaryMonitor);
        ...
        function onSeekingPrimaryMonitor(e) {
            var secondaryMonitor = document.querySelector(Component.Monitor.secondaryMonitor.selector + ' .video');
        ...
        function onSeekingSecondaryMonitor(e) {
            var primaryMonitor = document.querySelector(Component.Monitor.primaryMonitor.selector + ' .video');
            var currentTime = e.currentTarget.currentTime;
            if (primaryMonitor) {
    ...
    function openVideoDataFromMyDance(videoData) {
        ...
        if (videoData) {
            ...
            if (related.length > 0) {
                ...
                if (related[0].fileType == VideoSource.MY_VIDEO) {
                ...
                if (related[0].fileType == VideoSource.YOUTUBE) {
