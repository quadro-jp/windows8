...
(function () {
    ...
    WinJS.Namespace.define("Component", {
        ...
        DualMonitor: WinJS.Class.define(function DualMonitor(element) {
            ...
            setVideoPlayer: function(videoData){
                ...
                if (!videoData.primary && !videoData.secondary) {

                    var filename = Windows.Storage.ApplicationData.current.roamingSettings.values["lastTutorial"];

                    if (filename != '') {

                        var fileType = filename.indexOf('youtube') != -1 ? VideoSource.YOUTUBE : VideoSource.LOCAL_VIDEO;
                        videoData = {
                            primary: { fileType: fileType, name: filename },
                ...
                var relatedVideoListContainer = document.querySelector('.smallMonitor .RelatedVideoListContainer')
            ...
            setCountDown: function(){
                ...
                var countdown = new Jidori.Component.CountDown(Jidori.player.primaryMonitor.element, function (monitor) {
                    ...
                    function onCaptureStart(storageFile) {
                        ...
                        if (primaryMonitor) {
                            primaryMonitor.addEventListener('ended', onPrimaryMonitorComplete, false);
                    ...
                    function onCaptureStop(storageFile) {
                        if(Jidori.selector('.LIVE')) Jidori.selector('.LIVE').style.opacity = 1;
                        if (Jidori.selector('.REC')) Jidori.selector('.REC').style.opacity = 0;
                        ...
                        if (Jidori.player) {
                            MyDocument.userLibrary.getFileAsync(storageFile.name).done(function (item) {

                                var capturedVideoData = new Class.FileInformationParser([item], false);
                                Jidori.player.secondaryMonitor.bindingList.unshift(capturedVideoData);

                                //var videoData = new Class.VideoData(VideoSource.LOCAL_VIDEO, item, false);
                                //Jidori.player.secondaryMonitor.bindingList.unshift(videoData);
            ...
            removeCountDown: function () {
                ...
                try {
                    ...
                    Jidori.player.primaryMonitor.removeVideoPlayer();
                    Jidori.player.primaryMonitor.setVideoPlayer(videoData);
    ...
    function setVideoListPosition() {
        ...
        $('.largeMonitor .RelatedVideoListContainer').css({ top: -listContainerHeight, left: -listContainerWidth, 'opacity': 1 });
        $('.smallMonitor .RelatedVideoListContainer').css({ top: 20, left: 0, 'opacity': 1 });
    }
    ...
    function tryCapture() {
        ...
        try {
            ...
            Jidori.selector('#captureButton').querySelector('.win-label').textContent = '停止';
            ...
            Jidori.selector('#captureButton').querySelector('.win-label').textContent = '録画';
    ...
    function stopCapture() {
        ...
        try {
            ...
            isCaptureStart = false;
            Jidori.player.removeCountDown();
            Jidori.selector('#captureButton .win-label').textContent = '録画';
    ...
    WinJS.Namespace.define("Component", {
        ...
        BlankVideoPlayer: WinJS.Class.define(function BlankVideoPlayer(element, videoData, controls) {
            ...
            if (element.id == Component.Monitor.primaryMonitor.id) {
                ...
                inner.addEventListener('click', clickHandler, false);
...
(function () {
    ...
    WinJS.Namespace.define("Component", {
        ...
        LocalVideoPlayer: WinJS.Class.define(function LocalVideoPlayer(element, videoData, controls) {
            ...
            var volume = element.id == Component.Monitor.primaryMonitor.id ? 1 : 0;

            if (videoData.fileInformation) {

                var src = URL.createObjectURL(videoData.fileInformation ? videoData.fileInformation : videoData, { oneTimeOnly: true });

                if (element.id == Component.Monitor.primaryMonitor.id && src) {
                    video.src = src;
                    video.volume = volume;
                    video.play();
                    this.videoList = new Jidori.Component.VideoList(element);
                ...
                var tutorialFolder = Storage.getLatestRecordsFromTable(1, Jidori.tables.SETTINGS)[0].tutorialFolder;
                var videoList = this.videoList;
                var roamingSettings = Windows.Storage.ApplicationData.current.roamingSettings;

                try{
                    if (Jidori.player.primaryMonitor.videoData.name == '') {
                        Jidori.player.primaryMonitor.videoData.name = roamingSettings.values["lastTutorial"];
                    }
                } catch (e) { }

                if (MyDocument.tutorialLibrary) {
                    MyDocument.tutorialLibrary.getFilesAsync().done(function (item) {

                        var s = 0;

                        item.forEach(function (fileInformation) {

                            if (WinJS.Navigation.location == Jidori.pages.play.uri) {
                                if (fileInformation.name == roamingSettings.values["lastTutorial"]) {
                                    video.src = URL.createObjectURL(fileInformation, { oneTimeOnly: false });
                                    video.volume = volume;
                                    video.play();
                                }
                            }
                        ...
                        videoList = new Jidori.Component.VideoList(element);
                    });
                }
            }
...
(function () {
    ...
    WinJS.Namespace.define("Component", {
        ...
        YoutubeVideoPlayer: WinJS.Class.define(function YoutubeVideoDisplay(element, videoData, controls) {
            ...
            var embedURL = videoData.information ? videoData.information.embed : videoData.src;
            var iframe = document.createElement('iframe');
            container.appendChild(iframe);

            if (embedURL) {
                iframe.src = embedURL;
                Windows.Storage.ApplicationData.current.roamingSettings.values["embedURL"] = embedURL;
            } else {
                iframe.src = Windows.Storage.ApplicationData.current.roamingSettings.values["embedURL"];
            }
    ...
    function createListView(element) {
        ...
        listView.oniteminvoked = function (args) {
            ...
            args.detail.itemPromise.done(function (videoData) {
                ...
                if (Jidori.selector('#secondaryMonitor').hasClass('smallMonitor')) Jidori.player.toggleMonitor();
...
(function () {
    ...
    WinJS.Namespace.define("Component", {
        ...
        CountDown: WinJS.Class.define(function CountDown(element, callback) {
            ...
            video.controls = false;
            video.play();
            video.addEventListener('ended', callback ? callback : function () { Jidori.log('カウントダウン終了'); }, false);
        })
    });
